...
:local fJParseFunctions [file find name~"scriptRobot/.*/JParse.rsc"]
:local JParseName [file get value-name=name number=$fJParseFunctions]
import file-name="$JParseName"

:foreach fScript in=[file find name~"scriptRobot/.*/te.*"] do={
  :local fileName [file get value-name=name number=$fScript]
  :import file-name="$fileName"
}

:foreach fdbase in=[file find name~"scriptRobot/.*/dbase.*"] do={
